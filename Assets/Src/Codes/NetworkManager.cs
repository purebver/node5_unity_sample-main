...
public class NetworkManager : MonoBehaviour
{
    ...
    // 공통 패킷 생성 함수
    async void SendPacket<T>(T payload, uint handlerId, Packets.PacketType PacketType = Packets.PacketType.Normal)
    {
    ...
    void SendPingPacket(Ping data)
    {
        Debug.Log(data.timestamp);

        SendPacket(data, (uint)Packets.HandlerIds.Normal, Packets.PacketType.Ping);
    }
    ...
    {
        Ping data = Packets.Deserialize<Ping>(packetData);
        //Debug.Log($"Processed SpecificDataType: {data.timestamp}");
        SendPingPacket(data);
    }
    ...
    void HandleNormalPacket(byte[] packetData) {
        ...
        if (response.data != null && response.data.Length > 0) {
            switch (response.handlerId)
            {
                case 0:
                    {
                        LastPosition data = Packets.Deserialize<LastPosition>(response.data);
                        Debug.Log($"Processed SpecificDataType: {data.x}");
                        Debug.Log($"Processed SpecificDataType: {data.y}");
                        Debug.Log($"Processed SpecificDataType: {data.userId}");
                        GameManager.instance.GameStart();
                        GameManager.instance.player.UpdatePositionFromServer(data.x, data.y);
                        break;
                    }
            }
   ...
   /* void ProcessResponseData(byte[] data) {
        try {
            ...
            LastPosition response = Packets.Deserialize<LastPosition>(data);
            *//*Debug.Log($"Processed SpecificDataType: {response}");*//*
        } catch (Exception e) {
